...
def migrate(cr, version):
    ...
    if main_company_id:
        cr.execute(
            'UPDATE sms_template SET company_id = %s WHERE company_id IS NULL',
            [main_company_id],
        )
    ...
    cr.execute("""
        UPDATE mimsms_config
           SET retry_delay_minutes = COALESCE(NULLIF(retry_delay_minutes, 0), 5),
               max_sms_parts = COALESCE(NULLIF(max_sms_parts, 0), 6)
    """)
    _logger.info('Migrated MiMSMS templates to company-specific configuration')
